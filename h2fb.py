...
class MyHTMLParser(SGMLParser):
    ...
    def make_bins(self):
        ...
        #print ''.join(['<binary content-type="%s" id="%s">%s</binary>' % \
        ...
        return ''.join(['<binary content-type="%s" id="%s">%s</binary>' % \
                        (mime_type,x.encode('utf-8'),y) for mime_type, x,y in self.bins])
...
default_params = {
    ...
    'encoding-to'       : 'utf-8',       # Result data encoding
    ...
    'skip-ext-links'    : 1,        # Ignore external links
